...
class TestShow(TestFlaskBase):
    def test_show_empty_grade_query(self):
        ...
        response = self.client.get(url_for("grade.listOneAll"), headers=token)
    ...
    def test_show_by_grade_id(self):
        ...
        response = self.client.post(url_for("grade.create"), json=data1, headers=token)
        response = self.client.get(
            url_for("grade.listOneAll", grade_id=response.json["grade_id"]),
            headers=token,
        )
        self.assertEqual(1, len(response.json))
    ...
    def test_show_by_grade_name(self):
        ...
        response = self.client.post(url_for("grade.create"), json=data1, headers=token)
        response = self.client.get(
            url_for("grade.listOneAll", grade_name=response.json["grade_name"]),
            headers=token,
        )
        self.assertEqual(1, len(response.json))
