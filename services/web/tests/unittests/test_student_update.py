...
class TestStudentUpdate(TestFlaskBase):
    def test_update_grade(self):
        ...
        data_student.update(
            {"name": "Cleiton", "student_id": response.json["student_id"]}
        )
        response_update = self.client.put(
            url_for("student.update"), json=data_student, headers=token
        )

        data_student["created_updated_at"] = response_update.json["created_updated_at"]
    ...
    def test_update_grade_error(self):
        ...
        response_grade = self.client.post(
            url_for("grade.create"), json=data, headers=token
        )
        data_student = {
        ...
        data_student.update({"student_id": response_create.json["student_id"]})
